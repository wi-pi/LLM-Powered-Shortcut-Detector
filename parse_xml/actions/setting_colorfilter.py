...
def action_setting_colorfilter(elem):
    if not helper.check_validation(elem):
        return helper.append_data({'Turn': 'color filters On'}, None)
    ...
    invert = 'color filters'
    if operation == 'Turn':
        state = helper.get_elements_after_key(elem, 'state', 'integer')
        if state is not None:
            if state[0] == 0:
                invert = 'color filters Off'
            else:
                invert = 'color filters On'
        else:
