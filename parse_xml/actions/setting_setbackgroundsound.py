...
def action_setting_setbackgroundsound(elem):
    if not helper.check_validation(elem):
        return helper.append_data({'Turn': 'Background Sounds On'}, None)
    ...
    invert = 'Background Sounds'
    if operation == 'Turn':
        state = helper.get_elements_after_key(elem, 'state', 'integer')
        if state is not None:
            if state[0] == 0:
                invert = 'Background Sounds Off'
            else:
                invert = 'Background Sounds On'
        else:
