...
def action_move_notes(elem):
    if not helper.check_validation(elem):
        return helper.append_data({'Move': '', 'to': ''}, None)
    ...
    # Get the folder
    folder = ''
    folder_elem = helper.get_elements_after_key(elem, 'container', 'dict')
    if folder_elem is not None:
        folder = helper.parse_note(folder_elem[0])
