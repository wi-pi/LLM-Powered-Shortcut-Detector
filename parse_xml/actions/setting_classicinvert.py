...
def action_setting_classicinvert(elem):
    if not helper.check_validation(elem):
        return helper.append_data({'Turn': 'Classic Invert On'}, None)
    ...
    invert = 'Classic Invert'
    if operation == 'Turn':
        state = helper.get_elements_after_key(elem, 'state', 'integer')
        if state is not None:
            if state[0] == 0:
                invert = 'Classic Invert Off'
            else:
                invert = 'Classic Invert On'
        else:
            invert = 'Classic Invert On'
