            ...
            for filter_elem in filters_array:
                ...
                if data_b_dict_inter is not None and len(data_b_dict_inter[0]) > 0:
                    ...
                    if element_type == 'Number' or element_type == 'Enumeration':
                        ...
                        if data_b is None:
                            data_b = ''
                    elif element_type == 'ByteCountUnit':
                        # Get the value
                        value_here = helper.get_elements_after_key(data_b_dict_inter[0], 'Number', 'dict')
                        if value_here is None:
                            value_here = ''
                        else:
                            value_here = helper.get_final_value(value_here[0])
                            value_here = helper.get_attachments_by_range(value_here)
                        if value_here is None: value_here = ''
                        # Get the unit
                        unit_here = helper.get_elements_after_key(data_b_dict_inter[0], 'ByteCountUnit', 'dict')
                        if unit_here is None:
                            unit_here = ''
                        else:
                            unit_here = helper.get_final_value(unit_here[0])
                            unit_here = helper.get_attachments_by_range(unit_here)
                        if type(unit_here) is not list and unit_here != '':
